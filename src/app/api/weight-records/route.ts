...
const WRITABLE_FIELDS = ["cat", "measuredAt", "weightGrams", "note"] as const;

// GET /api/weight-records?catId=&from=&to=&limit=（只能看自己的貓）
export async function GET(request: NextRequest) {
  try {
    ...
    const records = await WeightRecordModel.find(filter)
      .sort({ measuredAt: -1 })
      .limit(limit)
      .populate("cat", "name")
...
// POST /api/weight-records（只能記錄自己的貓）
export async function POST(request: Request) {
  try {
    ...
    const record = await WeightRecordModel.create({ ...pick(body, WRITABLE_FIELDS), createdBy: userId });
